...
def create_rubiks_icon(size=256):
    ...
    def to_screen(x, y, z):
        # x is right-front, y is left-front, z is up
        sx = cx + (x - 1.5) * ux + (y - 1.5) * vx
        sy = cy + (x - 1.5) * uy + (y - 1.5) * vy - (z - 1.5) * wy
        return (sx, sy)

    # Base black cube / shadow
    margin = 0.08

    # TOP face (z = 3) - White / Yellow
    top_colors = [
    ...
    for ix in range(3):
        for iy in range(3):
            ...
            draw.polygon([b1, b2, b3, b4], fill="#151515")
    ...
    print("Icon generated successfully at assets/icon.png and icon.png")

if __name__ == "__main__":
    create_rubiks_icon(256)
